...
12. `Options.json` Version 2 adds Element and TEXTBOX Item `Availability`. Missing values and Version 1 data migrate as `ALWAYS`. PICTURE / HITBOX are Element-only. A controlled Item requires its parent Element to be available; parent disable retains child state. Option Effects may target any real Node, including from Global Events, and enabled state is saved but reset for a new game.
13. Do not rename stable Node, Event, Stat, Memory Bank, Element, Item, or Content IDs casually. Node, Element, and Item deletion must account for Option Effect references.
