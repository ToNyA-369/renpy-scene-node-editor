...
6. The fixed `__global__` Global Node is an authoring scope, not a Stack Node. It has no Options, cannot use `Action:` Triggers or Option Effects, and cannot be Root or Next Node. Its Events merge with the current real Node; End up operates on that real Stack-top context.
7. Event Content stores a Ren'Py label name, not an `.rpy` filename.
8. Event Effects cover Stats, Memories, and idempotent Option `enable` / `disable` operations and run before Content. Backgrounds, audio, transitions, and other presentation use native Ren'Py in Content, which should normally `return` to the Runner.
...
12. `Options.json` Version 2 adds Element and TEXTBOX Item `Availability`. Missing values and Version 1 data migrate as `ALWAYS`. PICTURE / HITBOX are Element-only. A controlled Item requires its parent Element to be available; parent disable retains child state. A Scene Node Option Effect may target only an Option owned by that same Node; cross-node and Global Event Option Effects are invalid. Enabled state is saved but reset for a new game.
13. Do not rename stable Node, Event, Stat, Memory Bank, Element, Item, or Content IDs casually. Element and Item deletion must account for same-Node Option Effect references.
14. Do not introduce new Schema fields or change public Runtime semantics without explicit creator approval.
15. Do not overwrite creator-owned `gui.rpy`, `screens.rpy`, or game data during framework updates.
