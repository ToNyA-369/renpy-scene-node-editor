...
Real Scene Nodes can be reordered by dragging the whole row. Dwell over another Node for about half a second; a framed group reservation opens below that card, and releasing creates a group just like the Event Pool, with up to three nested levels. Nodes cross group frames directly to move in or out; blank space beside a group name moves the whole group, and a one-Node group is retained; only empty groups disappear. Groups are collapsed by default and expand on hover, keyboard focus, drag entry, or while one of their Nodes is selected. Selecting a Node outside the group contracts the previous group along the reverse of its opening path instead of letting a redraw hide it instantly; a lingering hover cannot reopen it mid-collapse. Reordering inside a group keeps it open until the pointer actually leaves. Grabbing the whole group continuously contracts its floating preview from the expanded height to the heading. After dropping and saving, it expands from that heading height only when it contains the selected Node; otherwise it stays collapsed. The Global Node remains fixed at the top and does not participate. `Group`, `Group Path`, and `Order` affect authoring presentation only; they do not change ROOT, GOTO / REPLACE, graph depth, or Runtime execution.

The Node workspace uses the same full width and top spacing as the other workspaces. Its `Registered Tags` summary lists Tags registered by this Node through Memory `add` Effects and groups them by the creator-facing Memory Bank name. Technical Bank IDs are not repeated in this overview.
...
While the pointer travels from an expanded Node group to a neighboring group, traversed groups remain open for that sidebar-hover session and contract after the pointer leaves the group list, so the intended lower target does not move during acquisition.
...
- `Priority`: accepts 0–9; lower numbers win. New Events default to 5, and only the lowest matching layer is considered.
...
Event Memory Tag fields remain free text, so creators can still introduce a new Tag. On focus or while entering a prefix, the menu filters project-wide Tags registered by Memory `add` Effects in the currently selected Bank. Choose with the pointer or Arrow keys, Home / End, and Enter; Escape closes only the suggestions, while Tab continues the existing field order.

The Event Pool supports up to three nested levels of groups but keeps one full-width Add Event button; new Events appear visually ungrouped. While dragging, the current insertion gap follows the pointer and nearby Events or group blocks move up or down to make room. The preview stays attached to every pointer update while layout work is coalesced to one update per animation frame. A small midpoint hysteresis prevents insertion flip-flop, and approaching the list edges scrolls progressively without releasing the item. The flow keeps natural trailing space, so an Event can be placed after a bottommost group. Crossing out of a group returns the Event to its parent flow, or the loose flow at the root, while crossing into another group inserts it there; no dedicated ungroup target is required. Dwell over another Event's current bounds for about half a second; a framed group reservation opens below it before release. If live reflow moves that Event away from the pointer, the grouping intent is cancelled immediately. Groups are collapsed by default to a compact name field and item count, and expand on pointer hover, keyboard focus, or while their Event is being edited. Selecting an Event in another group or in the ungrouped flow contracts the previous group along the reverse opening path; hover remains disarmed until that contraction finishes and the pointer leaves. Reordering inside a group keeps it open until the pointer leaves. The unmarked blank space beside the name drags the entire group as one ordering block. Its preview contracts from the expanded contents to the heading; after saving, only a group containing the selected Event expands at the new position, while every other dropped group stays collapsed. A group stays intact when one Event remains; only empty groups disappear. Selecting a nested member opens all its ancestors. Whole subgroups can move into other groups after a short dwell or outside to a parent level, provided the entire subtree fits within three levels and never enters itself or a descendant. Group names are edited directly and order is stored in Editor-only `Order` metadata. Successful drags do not show a toast; failures restore the previous layout and remain visible.
...
Content entries and GOTO / REPLACE destinations show their relative chance beside each weight, updating as you type. Each list calculates independently: a single entry is 100%; multiple entries use each weight divided by their total. This is the chance of that Content or destination being selected once its Event is chosen. It is display-only and does not change stored weights or selection rules. Percentages use the standard row gap before the remove button, whose hover shape matches Conditions / Effects.

The Event Pool uses the same hover handoff: traversed groups remain open until the pointer leaves the group list, keeping a neighboring lower target stationary.

### Random Effect groups

Dwell one Effect over another to create a “Random choice” group. Its gray frame follows the Conditions style, but `RANDOM` means exactly one member runs, not AND. New members start at weight 1 (equal chances). Edit the weight on each row's right to see its resulting chance immediately: weights 1 and 3 give 25% and 75%; they do not need to sum to 100.

Drag members to reorder, move out, or join another group; the blank space beside the heading moves the whole block. Groups have only one level. A singleton stays grouped at 100%; only empty groups disappear. Add Effect still adds an ordinary leaf. The entire Effects list executes top-to-bottom, drawing when a group is reached and continuing after its selected member. Earlier changes are visible to later selected expressions. Existing undo, Tab / Escape, and Cmd+Enter behavior remains available; weights are focusable fields within each row.

Using groups automatically promotes that Event to Version 3. Update the project's Editor and FRAMEWORK together; legacy Effects do not auto-group or change execution order.
...
The Element sidebar and TEXTBOX Items in form mode can be reordered directly without a separate handle or grouping behavior. A TEXTBOX Item's whole card is the drag surface; only its remove button remains an independent control. A normal click that stays below the drag threshold switches the current Item directly, even immediately after typing in another Item's field. Explicit Z Order is the primary layer control in both the canvas and the game: a larger value is on top and receives overlapping clicks first. Element array order is persisted as the stable tie-breaker for equal Z Order, with the later entry on top. TEXTBOX Item order also determines game display and staggered entrance order.

Canvas mode keeps the preview and Inspector visible together at an approximate 4:3 width ratio. The Inspector header keeps the current Element, type, appearance summary, and Layout / Style / Effects / Item categories visible while one focused control set appears below; there are no nested disclosure cards or separate appearance page. Selecting a Text Box Item on the canvas opens that Item category directly, and color, typography, or effect changes remain visible on the canvas while editing. Profiles are individual JSON files under `game/DATA/TEXTBOX_PROFILES/` and provide base colors and text styling plus eleven optional features: hover accent, hover text color, Item border, rounded Item corners, horizontal text padding, bold text, italic text, letter spacing, text shadow, text outline, and staggered item entrance. The same profile can be reused by multiple Text Boxes in any Scene Node or Global Options scope; updating it updates every reference that has not overridden that value.
...
Dragging a whole Stat group shows only its heading and keeps the floating preview's left, right, and bottom frame intact. Member contents are fully hidden, and negative margins used only for in-list alignment do not carry into the floating layer.

### Memory Banks

Memory Banks store tags. Conditions use `has` / `not_has` for one tag, or `empty` / `not_empty` for the whole bank without a Tag value. Effects use `add` / `remove` / `clear`.
...
- Event Conditions, Effects, Content, and End up stay expanded and no longer have collapse controls. After creating an Event, type its Name directly, then press Tab through Trigger mode, Trigger value, Priority, Weight, and Once before moving across the four sections at one hierarchy level. Enter toggles a focused Once field. On a focused section, Enter enters its existing fields; `Cmd/Ctrl + Enter` adds the corresponding child and enters the new item. Tab/Shift+Tab stays within that item's field order and advances to the next section only after the last field; Escape returns to the containing section. Lifecycle Events skip the absent Weight and End up controls. Autosave-driven rerenders preserve the current focus.
- Every dropdown, including Event Content labels, uses the same shared component with uniformly sized option rows. A menu grows to match the number of items at the current level; only layers above the visible limit scroll. After opening any dropdown, Up/Down navigates the current level and Home/End moves to its first or last item. Focusing a parent item reveals its submenu, but focus enters it only after Right; Left returns to the parent, Enter selects, and Escape first closes the menu.
