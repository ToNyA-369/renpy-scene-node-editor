...
Scene Node 列表中的實際節點可整列拖移排序；在另一個節點上停留約半秒，目標卡片下方會展開群組預留空間，放開即可像 Event Pool 一樣建立最多三層的巢狀群組。節點可直接跨群組框移入或移出，群組名稱旁的空白可拖移整組，只剩一個節點時仍保留群組，空群組才消失。群組預設收起並在 hover、鍵盤 focus、拖移進入，或其節點目前正被選取時展開；改選群組外的節點後，舊群組會沿展開的反向路徑縮合，而不是在重繪時瞬間消失，收合途中也不會被仍停留的 hover 撐開。在群組內排序後會保持展開，直到游標真正離開。抓取整組時，浮動預覽會從目前展開高度連續縮合為標題；放到新位置並保存後，只有目前選取節點位於該群組時才從標題高度重新展開，其他群組保持收合。Global Node 固定在頂端，不參與排序或群組。`Group`、`Group Path` 與 `Order` 只影響 Editor 中的創作管理，不改變 ROOT、GOTO／REPLACE、關聯圖深度或 Runtime 執行。
...
使用滑鼠從已展開的節點群組移往相鄰群組時，經過的群組會在這次側欄 hover 期間保持展開；離開群組清單後才收合，避免下方目標在瞄準途中位移。
...
- `End up`：Content 返回後執行 REDO、GOTO、REPLACE 或 EXIT。GOTO／REPLACE 都可使用單一或權重 Next Node；選單會以「群組 → 子群組 → 節點」呈現已群組節點，未群組節點維持第一層，保存值仍是穩定 Node ID。
...
Event Pool 使用最多三層的巢狀群組整理大量 Events，但只保留一個展滿側欄的新增 Event 按鈕；新 Event 預設不顯示群組。拖移時目前插入位置會直接騰空，周圍 Event 與群組區塊隨游標向上或向下讓位；預覽會持續跟住游標，實際排序每畫面幀最多更新一次。游標在元素中線附近的小幅晃動不會讓插入位置反覆翻轉，靠近清單上下邊緣時會漸進自動捲動。排序流末端也保留自然留白，因此 Event 可放在最末群組之後。移出原群組邊界即回到父群組排序流（最外層則回到未群組區），進入另一個群組邊界則可直接插入，不需要專用的移出位置。在另一個 Event 的目前邊界上持續停留約半秒，目標下方會展開帶框的群組預留空間；若對方已被讓位動畫推離游標，停留意圖立即取消，不會在放開時誤成組。群組預設收起，只顯示較短的名稱欄與數量；游標移入、鍵盤焦點進入，或目前正在編輯群組內的 Event 時展開。改選其他群組或未群組 Event 後，舊群組會沿展開的反向路徑縮合，動畫完成且游標離開前不重新接受 hover 展開；在群組內完成排序後仍保持展開，直到游標離開。名稱旁沒有圖示的空白可拖移整個群組，起拖時浮動預覽會從展開內容連續縮合成標題；落位保存後只有包含目前選取 Event 的群組才以同一路徑展開，其餘維持收合。群組只剩一個 Event 時仍保留，空群組才消失。選中深層成員時，所有上層群組都保持展開。整個子群組可拖到其他群組上停留後移入，或拖出移回上層；整棵子樹必須維持在三層內，且不能移入自己或自己的後代。群組名稱可直接點擊修改，順序保存於只供 Editor 使用的 `Order`；成功拖移不顯示額外通知，保存失敗時才顯示錯誤並回復原狀。
...
Content 演出與 End up 的 GOTO／REPLACE 目標都會在權重旁即時顯示相對機率：單一項目為 100%，多個項目以各自權重除以總權重換算。兩份清單各自計算，表示該 Event 被選中後，抽到該演出或目標的機率；僅為顯示，不改變權重資料與抽選規則。百分比與刪除按鈕保持相同的列間距，刪除按鈕的 hover 外形與 Conditions／Effects 一致。

從已展開 Event 群組移往相鄰群組時，經過的群組同樣會保持展開到游標離開群組清單，讓下方目標維持原位。

### 效果的機率群組

將一個 Effect 拖到另一個上停留，便可建立「隨機擇一」群組。灰色群組框沿用 Conditions 的樣式，但 `RANDOM` 表示每次只觸發其中一項，不是 AND。新成員預設權重為 1（等機率）；直接在每列右側修改權重，旁邊會即時顯示換算後的機率。例如 1 與 3 代表 25% 與 75%，不需合計為 100。

可拖移成員排序、移出或移入其他群組；標題旁的空白可拖動整組。群組始終只有一層，剩一項時保留並顯示 100%，空組才消失。新增按鈕仍只加入普通效果。整張 Effects 清單由上到下執行，走到群組才抽選一項、執行完再往下；先前效果改動的值會影響後續抽中效果的運算。可用原有復原、Tab／Esc 與 Cmd+Enter 操作，權重也是列內可聚焦欄位。

使用機率群組的事件會自動升為 Version 3，請同步更新專案的 Editor 與 FRAMEWORK；舊事件不會自動成組，也不改變原有順序。
...
拖移整個 Stat 群組時，浮動預覽只顯示群組標題，會保留完整左右與底部外框；成員內容完整隱藏，只供列表對齊的負邊距也不會帶入浮動層。

### Memory Banks

Memory Banks 保存標籤。Conditions 以 `has`／`not_has` 判斷單一標籤，也能以不需 Tag 值的 `empty`／`not_empty` 判斷整個記憶庫；Effects 使用 `add`／`remove`／`clear`。
