...
第一次建立圖面時會採用漸進式拓撲播種：先放置 ROOT，再逐層建立可達的 GOTO 骨架，之後才把同層 REPLACE 家族加入已預留的父層圓環；若 REPLACE 節點還有自己的 GOTO 子節點，會在下一輪繼續向外生長。無法從 ROOT 推導的節點最後才加入。這些階段會在顯示前快速完成，使用者看到的是已接近平衡的完整圖，而不是需要等待的播放動畫；相同資料與 ROOT 會得到可重現的初始結果。
...
為減少視覺打結，力場只對沒有共用端點、而且實際交叉的線加入微弱交叉懲罰，不會把所有靠近或平行的線互相推開。主幹 GOTO 優先保持穩定，REPLACE 與一般跨支線其次，半透明管理邊優先讓路；ROOT、分支中心和正在拖曳的節點也不會被輔助線突然拉走。交叉調整會逐步改變子節點的局部圓環角度，非樹狀線則優先向整體圖形外側彎曲。這些力量在拖曳後仍會重新運作，但只屬於本次檢視，不保存位置或改變流程資料。
