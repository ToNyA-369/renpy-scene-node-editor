...
5. Node 表單應只有 Name 與 ID，不應出現 Background 或 Screen；工作區卡片需對齊其他功能區的左右邊界與頂部間距。下方顯示 Events、Options、Content Labels、Flow Links 數量，以及 Incoming／Outgoing、三個生命週期階段與 `Registered Tags` 摘要；Tags 依 Memory Bank 的創作者名稱分組，不重複顯示穩定 ID。相同目標與流程類型的多個 Events 應合併為一個連接標籤並顯示倍數。開啟包含 Memory Condition／Effect 的 Event，在 Tag 欄輸入前綴時應出現與共用下拉選單同尺寸的專案候選，並可用滑鼠、方向鍵與 Enter 選取；仍可直接輸入不存在的新 Tag。開啟 `root_enter_background`、`root_on_node_once`、`root_exit_cleanup` 三個 Events，Auto 時機應分別顯示 On Enter、On Node、On Exit。On Enter／On Exit 只顯示 Priority 與 Once，不應出現 Weight、End up、Next Node 或額外提示文字；Conditions、Effects 與 Content 仍存在。
