...
- Option 可為常駐 `Always` 或由 Effect 啟用／停用的 `Controlled`；TEXTBOX 可控制整列或個別 Item，且只能由同一 Options 作用域的 Event 控制。
- Content 保存的是 Ren'Py `label` 名稱，不是 `.rpy` 文件名。
- On Enter／On Exit 可在節點邊界依序執行多個 Events；On Node 沿用原本 Auto 的單一選擇。
- 固定且不可刪除的 Global Node 提供全域 Events 與 Options；它不進入 Stack，但其 Options 會與目前 Scene Node 的 Options 一起顯示，並可觸發或由同作用域的 Global Events 控制。
